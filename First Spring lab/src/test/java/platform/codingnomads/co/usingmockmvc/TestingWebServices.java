...
public class TestingWebServices {
    ...
    @Test
    public void confirmCorrectViewAlbert() throws Exception {
        mockMvc.perform(
                        get("/albert")
                )
                .andExpect(status().isOk())
                ...
                .andExpect(view().name("greeting"))
                //expect the model to include two attributes called name and username
                .andExpect(model().attribute("name", "Albert"));
    }

    @Test
    public void confirmEmptyReturn() throws Exception {
