...
public class JpaRepoDemo implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        SoftDrink mtDew = SoftDrink.builder().name("Mountain Dew").rating(8).build();
        SoftDrink mlwYellow = SoftDrink.builder().name("Mellow Yellow").rating(5).build();

        //save single entity instance
        fanta = softDrinkRepo.save(fanta);
        mtDew = softDrinkRepo.save(mtDew);

        //save multiple entity instances at a time
        List<SoftDrink> insertedSoftDrinks = softDrinkRepo.saveAll(List.of(coke, drPepper, mlwYellow));
        ...
        List <SoftDrink> list = softDrinkRepo.findAll();
        for(SoftDrink drink: list){
            if(drink.getRating() == 4) {
                System.out.println("Soda " + drink.getName() + " has a rating of 4.");
            }
        }
        ...
        softDrinkRepo.delete(mlwYellow);
