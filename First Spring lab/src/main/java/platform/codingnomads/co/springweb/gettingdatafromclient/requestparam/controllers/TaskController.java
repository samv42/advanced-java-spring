...
public class TaskController {
    ...
    @GetMapping(value = "/example1", produces = MediaType.APPLICATION_JSON_VALUE)
    public String getName(@RequestParam String name){
        return "Name: " + name;
    }
    @GetMapping(value = "/example2", produces = MediaType.APPLICATION_JSON_VALUE)
    public String getNameOptional(@RequestParam(required = false, defaultValue = "John") String name){
        return "Name: " + name;
    }
    @GetMapping(value = "/example3", produces = MediaType.APPLICATION_JSON_VALUE)
    public List<Long> getIds(@RequestParam List<Long> id){
        return id;
    }
}
